...
даёт `"a=q {}"` (важно! в `xformat()` используется для escape две
фигурные скобки). Кроме того, `xformat()` позволяет включать
в текст результат выполнения произвольных ECMAScript-выражений:
...
```js
{
  ...
  "alarms": [
    ...
    },
    {
      ...
      // на текущее значение контрола.
      "alarmMessage": "Temperature out of bounds, value = {}",
