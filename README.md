...
Rule engine for Wiren Board 2.0
...
- [Правила](#Правила)
  ...
  - [Доступ топикам meta](#Доступ-топикам-meta)
  - [API создания/управления устройств](#API-создания/управления-устройств)
...
- [Изоляция сценариев](#Изоляция-сценариев)
  - [Пример](#Пример)
  - [Совместимость с версией 1.7](#Совместимость-с-версией-1.7)
...
`name` - имя правила, которое должно быть уникальным в рамках текущего файла.

В анонимных правилах имя можно не задавать, тогда оно сгенерируется автоматически:
```js
defineRule({
...
`startTicker()` ниже). При задании `whenChanged` правило срабатывает при любых изменениях значений параметров или функций, указанных в списке.
...
аргументов текущее значение параметра, имя устройства и имя параметра, изменение которого привело к срабатыванию правила. 
...
с изменением параметра (например, вызов при инициализации, по таймеру или через `runRules()`), `then` вызывается без аргументов, т.е. значением всех трёх аргументов будет `undefined`.
...
В стандартной конфигурации, т.е. при использовании syslog, сообщение попадает `/var/log/syslog`, `/var/log/daemon.log`.
...
`trackMqtt(topic, callback())` подписывается на MQTT с указанным topic'ом, допустимы символы `#` и `+` значения передаются в функцию объектом состоящим из: .topic и .value.
Пример:
```js
...
* `/etc/wb-rules-modules` — сюда можно складывать пользовательские модули, они сохранятся при обновлении контролера.
