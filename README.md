...
Начиная с версии 3.0 сценарии выполняются движком
...
ES-модули (`import`/`export`) в файлах сценариев поддерживаются — и в
`.ts`, и в `.js`; такой файл выполняется как настоящий ES-модуль (см.
[ES-модули](#es-модули)). Импортировать можно модули из каталогов модулей
(`import { helper } from "my-helper"`), в том числе написанные на TypeScript,
и файлы по относительному пути (`import { f } from "./lib/util.ts"`).
Фоновая проверка типов разрешает такие импорты по тем же правилам, что и
движок, так что экспортированные модулем типы и сигнатуры проверяются
по-настоящему.
...
3.0 это был единственный вариант. Такой код работает без изменений; в новых
...
* Относительный путь в `require()` — `require("./x")` — разрешается относительно
  *имени модуля*, а не файла: из файла сценария это то же, что `require("x")`
  (поиск по каталогам модулей); из модуля `dir/a` — это `dir/x`. Так работало
  всегда; путь относительно файла на диске даёт `import` (см. ниже).
* Имя модуля можно указывать с расширением или без: `require("x")` найдёт
  `x.js`, а если его нет — `x.ts` (модуль на TypeScript транспилируется при
  подключении так же, как файл сценария).

### ES-модули

Файл сценария или модуля, в котором есть объявления `import`/`export` (или
`import.meta`), выполняется движком как настоящий ES-модуль — с живыми
привязками импортов, `import.meta` и «родным» `await` на верхнем уровне.
Формат `.js`/`.ts` определяется по исходному тексту (как в Node.js 22.7+ без
`package.json`): файл без таких объявлений — обычный сценарий
(CommonJS-подобная обёртка с `module`, `exports`, `require`), как и прежде.
Расширение может задать формат явно, как в Node.js: `.mjs`/`.mts` — всегда
ES-модуль (строгий режим, `import.meta`, без `module`/`exports`, даже если в
файле нет ни одного `import`/`export`), `.cjs`/`.cts` — всегда обычный
сценарий (модульный разбор не пробуется вовсе). Все шесть расширений
загружаются из каталогов сценариев и находятся в каталогах модулей;
`.ts`/`.mts`/`.cts` — TypeScript.

```js
// /etc/wb-rules/lights.js
import { schedule } from "sun-times";        // модуль из каталога модулей (JS или TS)
import { clamp } from "./lib/math.ts";       // файл по относительному пути
import Notify from "wb-notify";              // CommonJS-модуль: module.exports как default
import * as cfg from "./lib/config.js";      // пространство имён
export const version = "2";                  // экспорт (виден другим файлам через import)

const times = await schedule("today");       // top-level await

defineRule("evening", {
  whenChanged: "sun/elevation",
  then: (v) => { dev["lamp/enabled"] = clamp(v, 0, 1) < 0.1; },
});
```

Разрешение имён в `import` (и в `import()`):
* `"./x"`, `"../x"` — относительно каталога файла, в котором написан импорт
  (как в Node.js): сценарий может импортировать файл рядом с собой или в
  подкаталоге `/etc/wb-rules`;
* `"/abs/path/x.js"` — абсолютный путь;
* `"x"`, `"dir/x"` — поиск по каталогам модулей (см. Расположение), как у
  `require()`.

Имя с расширением `.js`/`.ts`/`.mjs`/`.mts`/`.cjs`/`.cts` берётся как есть
(`"./x.js"` при отсутствии `x.js` найдёт `x.ts`, `.mjs` → `.mts`, `.cjs` →
`.cts` — соглашение TypeScript), без расширения — пробуются `.js`, `.ts`,
`.mjs`, `.mts`, `.cjs`, `.cts` по очереди. Другие типы файлов (`.json` и т. п.)
модулями не являются.
Модули на TypeScript транспилируются при импорте; номера строк в трейсбеках
указывают на исходник.

Семантика экземпляров та же, что у `require()`: у каждого файла сценария —
свой экземпляр импортированного модуля (модуль выполняется заново в каждом
сценарии), а внутри одного сценария файл — один экземпляр, как бы он ни был
подключён: `require("x")`, `import "x"` и `import "/abs/x.js"` дают одно и то
же. Модуль, импортированный из ES-модуля, выполняется во время загрузки
импортирующего файла: сначала CommonJS-зависимости (при связывании), затем
ES-зависимости в порядке импортов; ошибка в любом из них — ошибка загрузки
файла (с трейсбеком модуля), как и отсутствующий модуль
(`cannot find module "x" (imported from /etc/wb-rules/a.js)`).

Совместимость в обе стороны:
* `import` CommonJS-модуля (`exports.x = …` / `module.exports = …`): `default` —
  это `module.exports`, именованные экспорты — его собственные перечислимые
  свойства на момент подключения (снимок, как в Node.js);
* `require()` ES-модуля возвращает его пространство имён (`m.default`,
  `m.x`); модуль с `await` на верхнем уровне так подключить нельзя —
  ошибка с кодом `ERR_REQUIRE_ASYNC_MODULE`, используйте `import` или
  `await import("x")`. `import()` работает и в обычных (не модульных) файлах.

`import.meta` ES-модуля: `url` (`file://…`), `filename`, `dirname` — путь самого
модуля (у `__filename` по-прежнему имя файла сценария — вершины дерева), и
`static` — хранилище, общее для всех экземпляров этого файла модуля (аналог
`module.static`; файл, подключённый и через `require`, и через `import`, видит
одно и то же хранилище). В ES-модуле нет `module` и `exports`; глобальные
функции wb-rules (`defineRule`, `log`, `require`, …) доступны как обычно.
Код ES-модуля выполняется в строгом режиме (`"use strict"`): присваивание
необъявленной переменной — ошибка, а не глобальная переменная.

Если модуль нужен только ради побочного эффекта, импортируйте его как
`import "x"`: неиспользуемый `import x from "x"` компилятор TypeScript
выбрасывает (в `.js` — нет). В редакторе правил импорты типизируются по
файлам контроллера (RPC `Editor.ResolveModule`; отдаются только файлы из
каталога правил и каталогов модулей), фоновая проверка на контроллере — по
каталогам модулей и соседним файлам. Замечания проверки внутри самого
импортированного модуля не выводятся (их не к какому файлу отнести);
неправильное использование модуля в сценарии — выводится.
