...
**Замечание об edge-triggered/level-triggered правилах:** Если в функции-условии используются обращения к контролам, то в целях оптимизации правило не будет срабатывать при каждом просмотре, а только при изменениях значений используемых контролов. Если правило срабатывает из-за изменения одного из этих контролов, то в функцию `then` передаются соответсттвующие аргументы `newValue`/`devName`/`cellName`, в остальных случаях `then` вызывается без аргументов.
