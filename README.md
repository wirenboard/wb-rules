...
`PersistentStorage` - не простой объект, он является прокси сущностью и поэтому при работе с ним не поддерживается вывод всего хранилища. Работать с сохраненными данными можно только после доступа через ключ:
...
при работе с ним не поддерживается вывод всего хранилища `ps` - при таком
обращении вы не получите никаких данных. Работать с сохраненными данными можно
только после доступа через ключ `ps['key2']`:
...
Обратите внимание: можно добавлять только элементы с простыми типами, но нельзя использовать объекты. Вместо `ps['key'] = {...}` нужно использовать `ps[idPrefix] = new StorableObject({...});`
...
// - Сначала создаем инстанс явно создавая объект с помошью StorableObject()
//   пустой или уже заполненный объект не важно
// - Далее уже можем работать с инстансом объекта как с обычным объектом
