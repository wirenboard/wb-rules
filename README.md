...
**Важно:** `trackMqtt()` не нужен для топиков вида `/devices/<устройство>/controls/<контрол>` — это параметры устройств, за которыми wb-rules следит сам. К ним относятся контролы всех устройств из раздела «Устройства» веб-интерфейса — модулей Wiren Board, драйверов `wb-mqtt-*` и виртуальных устройств, созданных через `defineVirtualDevice()`. Такие параметры доступны в правилах по имени `"устройство/контрол"`: изменения отслеживаются через `whenChanged`, а значение читается и записывается через объект `dev`, о которых рассказано выше. `trackMqtt()` предназначен для топиков вне этого дерева — например, от сторонних сервисов, которые не следуют Wiren Board MQTT Conventions.

Пример — подписка на топик стороннего сервиса:
```js
// Сторонний LoRaWAN-шлюз публикует заряд батареи датчика
// в свой топик, не по конвенции Wiren Board
trackMqtt("/lora/sensor1/battery", function(message) {
  log.info("name: {}, value: {}".format(message.topic, message.value));
});
```

Для сравнения — то же самое для параметра устройства делается правилом:
```js
// Напряжение питания контроллера — это контрол устройства wb-adc,
// trackMqtt() здесь не нужен
defineRule("vin_changed", {
  whenChanged: "wb-adc/Vin",
  then: function(newValue) {
    log.info("Vin: {}", newValue);
  }
});
```
