...
Если у контрола отсутствует определенный meta-топик, то при попытке чтения будет возвращено `undefined`. При попытке чтения meta-топика несуществующего контрола будет возвращено `null`.
...
При записи `null` или `undefined`, значение удаляется из хранилища, последущие чтения будут возвращать `undefined`.
