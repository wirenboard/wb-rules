...
Синтаксис получнеия доступа к значению мета-топика выглядит следующим образом: `dev["wb-mr3_48/K1#error"]` или `dev["wb-mr3_48/K1#writable"]`
...
Для отслеживания изменения значений также доступен триггер `whenChanged`. Например, для отправки смс как при потери так и восстановления связи с реле:
```
defineRule("onChange", {
