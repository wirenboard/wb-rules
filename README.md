...
Формат определяется по исходному тексту: файл без таких объявлений — обычный
сценарий (CommonJS-подобная обёртка с `module`, `exports`, `require`), как и
прежде. Никакого переименования (`.mjs`) или настройки не нужно.
...
Имя пробуется как есть, затем с `.js` и `.ts`; `"./x.js"` при отсутствии
`x.js` найдёт `x.ts` (соглашение TypeScript). Модули на TypeScript
транспилируются при импорте; номера строк в трейсбеках указывают на исходник.
...
файлам контроллера (RPC `Editor.ResolveModule`), фоновая проверка на
контроллере — по каталогам модулей и соседним файлам.
