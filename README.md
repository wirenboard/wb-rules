...
`Notify.sendApprise(target, title, text[, callback])` отправляет уведомление
через утилиту [Apprise](https://appriseit.com/) — единый интерфейс к более чем
150 сервисам уведомлений (ntfy, Telegram, Discord, Slack, Matrix, e-mail,
SMS-шлюзы и т.д.). Параметры:

* `target` — обязательный. Либо URL сервиса в синтаксисе Apprise, например
  `ntfy://ntfy.sh/mytopic` или `tgram://<токен бота>/<id чата>` (полный список
  схем — [appriseit.com/services](https://appriseit.com/services/)), либо тег
  из конфигурационного файла Apprise (`/etc/apprise`, `/etc/apprise.yml` —
  см. [документацию Apprise](https://appriseit.com/config/)); тегом можно
  отправить сообщение сразу в несколько каналов одним вызовом;
* `title` — заголовок уведомления (необязателен, может быть пустой строкой);
* `text` — текст уведомления.

Требуется установленный пакет `apprise`; он доступен начиная с Debian 13
(trixie) и ставится автоматически как рекомендация пакета wb-rules, вручную —
`apt-get install apprise`. Если утилита не установлена, при попытке отправки
в лог выводится подсказка об установке; остальные каналы уведомлений при этом
продолжают работать.

Эта функция используется для типа получателя `apprise` в сервисе алармов
(см. ниже).
...
```js
{
  ...
  "recipients": [
    ...
    {
      // Тип получателя - уведомление через утилиту Apprise
      // (150+ сервисов: ntfy, Telegram, Discord, Slack и т.д.).
      // Требуется установленный пакет apprise (Debian 13+).
      "type": "apprise",

      // URL сервиса в синтаксисе Apprise (полный список схем -
      // https://appriseit.com/services/) либо тег из конфигурационного
      // файла Apprise (/etc/apprise) - тогда сообщение уйдёт сразу во все
      // каналы с этим тегом.
      "url": "ntfy://ntfy.sh/mytopic",

      // Заголовок сообщения (необязательное поле).
      // {} заменяется на текст сообщения.
      "title": "Тревога Wiren Board"
    }
  ],
