...
примет предыдущее сохранённое значение (если оно существует; для новых виртуальных
устройств будет записано значение по умолчанию). Для того, чтобы вернуть старое поведение
...
Функция `defineVirtualDevice()` возвращает объект, представляющее собой виртуальное устройство.
Также этот объект можно получить с помощью глобальной функции `getDevice(<id девайса>)`.
У этого объекта есть методы, которые позволяют выполнять действия над этим устройством:
* `getDeviceId()` - возвращает строку id девайса переданного при его создании
* `getCellId(<id контрола>)` - возвращает строку, составленную из id девайса и id сонтрола, удобно использовать для доступа к значению контрола через переменную `dev[cellID]`
* `addControl(<id контрола>, {описание параметров})` - добавляет новый контрол к девайсу, принимает id контрола и описание параметров, аналогичное функции `defineVirtualDevice()`.
* `removeControl(<id контролало>)` - удаляет контролов по его id
* `getControl(<id контрола>)` - возвращает объект контрола (см. ниже) по его id
* `isControlExists(<id контрола>)` - возвращает `true` если контрол на девайсе присутствует, `false` в ином случае
* `controlsList()` - возращает массив всех контролов в данный момент присутствующих у девайса

У объектов контролов есть следующие методы:
* `getId()` - возвращает строку, содержащую id контрола
* `setDescription(<description>)` - устанавливает переданную строку в качестве поля description контрола
* `getDescription()` - возвращает строку, содержащую description контрола
* `setType()` - устанавливает переданную строку в качестве поля type контрола
* `getType()` - возвращает строку, содержащую type контрола
* `setUnits()` - устанавливает переданную строку в качестве поля units контрола
* `getUnits()` - возвращает строку, содержащую units контрола
* `setReadonly()` - устанавливает переданное булевое значение в качестве поля readonly контрола
* `getReadonly()` - возвращает булевое значение, содержащее readonly контрола
* `setMax()` - устанавливает переданное число в качестве поля max контрола
* `getMax()` - возвращает число, содержащее поле max контрола
* `setError()` - устанавливает переданную строку в качестве поля error контрола
* `getError()` - возвращает строку, содержащую поле error контрола
* `setOrder()` - устанавливает переданное число в качестве поля order контрола
* `getOrder()` - возвращает число, содержащее поле order контрола
