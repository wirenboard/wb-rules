...
```
Второй аргумент `{ global: true }` делает хранилище глобальным для всех правил.
Это означает, что если в разных файлах правил создать хранилища с одинаковыми именами,
они будут работать с одним и тем же объектом данных.
Порядок доступа к глобальному хранилищу из разных правил заранее не определен.

*Примечание:* Движок wb-rules обрабатывает правила последовательно (в одном потоке),
поэтому параллельный доступ к данным невозможен.
Два правила не могут обратиться к хранилищу в одно и то же время - выполнение всегда идет друг за другом.
Благодаря этой особенности, при работе с хранилищами в wb-rules не нужно использовать примитивы синхронизации (блокировки, мьютексы и т.п.),
привычные для многопоточного программирования.
