...
// quiet engine, which is why the shutdown races found by the wild-script
// corpus (esWbSpawn CallSync panic, MaybeCallSync running JS on a foreign
...
`
	...
	for i := 0; i < 5; i++ { // several rounds to catch the timing window
		...
		// in flight, then stop mid-activity
