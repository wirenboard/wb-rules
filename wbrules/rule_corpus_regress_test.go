...
// Synthetic regressions distilled from a 663-script wild-corpus run
// (wiki/forum scripts loaded through the engine). Each test is the
// minimal reproduction of a bug that the corpus surfaced and the
// regular suites missed; the corpus itself is not distributed.

// StrictRegressSuite: wild scripts use 'use strict'; every proxy set
...
// bareCorpusEngine mirrors how wild scripts met the engine: no prior
...
// A wild script's very first action was trackMqtt(): the subscribe path
