...
`enable`/`disable` идемпотентны и переименовывают файл в `<path>.disabled`
и обратно только когда состояние действительно меняется. Новое состояние
редактор узнаёт от своего наблюдателя файлов чуть позже, поэтому `enable`
сразу после `disable` того же файла может увидеть его ещё включённым и
ничего не сделать — между противоположными переключениями одного файла
дайте драйверу мгновение (например, `sleep(500)`).
