...
| **MqttRpc** | Встроенный модуль `wb-mqtt-rpc.js` (глобал `MqttRpc` = `require('wb-mqtt-rpc')`, экземпляр на realm): клиент MQTT-RPC (`call/hasMethod/waitForMethod/service`, `RpcError/TimeoutError`), типизированные обёртки сервисов контроллера (`serial`, `db`, `rules`, `confed`, `logs`, `diag`, `deviceManager`, `dali`) и сервер (`defineService`, топики доступности очищаются через `_wbAddCleanup`). |
