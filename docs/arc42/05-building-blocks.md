...
Раздел описывает статическую декомпозицию wb-rules (ветка `quickjs-ts`, пакет `3.0.0~quickjs2`): сначала контейнеры/процессы на контроллере и в браузере, затем пакеты внутри процесса `wb-rules`, затем два самых важных блока — шим `quickjsduk` и TS-loader — до уровня функций. Обоснования решений — в ADR (`см. ADR-NNN`).
...
| **Модуль `fs`** | Встроенный модуль файловой системы (ADR-017): `require("fs")`/`"fs/promises"`/`node:`-варианты разрешаются в `ModSearch` раньше каталогов модулей; тело `fsmodule.js` (`go:embed`) компилируется в realm каждого файла; Go-builtin'ы `_wbFsSync`/`_wbFsAsync` (таблица операций `fsOps`, Node-формы ошибок `code/errno/syscall/path/dest`), `_wbFsWatch`/`_wbFsWatchClose` (inotify через `x/sys/unix`: один экземпляр на движок, `fsWatchState{byID, byWd, byCtx}`, watch на inode делится между watcher'ами; закрытие из `runCleanups` файла (per-realm, как таймеры), в `Stop`, из JS и по `IN_IGNORED`) | `fsmodule.go`, `fsmodule.js`, `fswatch.go` | JS: `fs.*Sync`, `fs.*` → `Promise`, `fs.watch`; `types/wb-rules.d.ts` (`declare module "fs"`) |
...
graph LR
    subgraph TSC["TSCompiler"]
        TR["Transpile(src, path)<br/>persistent child: tsgo --api --async<br/>LSP-framed JSON-RPC transpileModule<br/>target ESNext, module CommonJS, esModuleInterop, sourceMap"]
        ...
        CM["checkMany: tsgo --noEmit --pretty false --target esnext --lib esnext<br/>--strict false --module preserve --moduleDetection force --esModuleInterop<br/>--allowJs --checkJs <paths> wb-rules.d.ts /tmp/wb-controls-*.d.ts<br/>60 s ctx timeout, Pdeathsig, tsDiagRx"]
