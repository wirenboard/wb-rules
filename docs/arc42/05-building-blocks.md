...
graph TD
    ...
    subgraph wbrules["пакет wbrules/"]
        ...
        ESE["ESEngine (esengine.go)<br/>Go-builtins, realm per file, loadScript,<br/>ModuleHost, PersistentStorage, vdev glue"]
...
- **Что имитирует.** Публичный API `go-duktape` (стек индексов, `PushGlobalObject`, `GetPropString`, `PutPropString`, `PushGoFunc`, `Pcall`, `PcallProp`, `PcompileStringFilename`, `PushHeapStash`, `PushThreadNewGlobalenv`, `Enum/Next`, CommonJS `require` через `ModuleHost` (бывший `Duktape.modSearch`), ES-модули (`CompileScriptOrModule`, `EvalModuleNoPump`, загрузчик `goModuleNormalize`/`goModuleLoad`), точные строки ошибок вида `Error: error error (rc -100)`). Код `wbrules/` почти не изменился при смене движка (см. ADR-003).
