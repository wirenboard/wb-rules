...
1. **Формат `.js`/`.ts` определяется по исходнику**; расширение может задать его явно
   (`.mjs`/`.mts` — модуль без попытки обёртки, `.cjs`/`.cts` — обёртка без попытки модульного
   разбора; `moduleFormatByName` в шиме, `jsExts`/`tsExts` в движке, регулярка watcher'а в
   `main.go`).
...
| Формат *только* по расширению (`.mjs`/`.mts`) | пользователи ожидают, что `import` в `.js`/`.ts` просто работает (так делает TS-проверка); явные расширения добавлены *в дополнение* к детекции - как в Node.js 22.7+ |
...
- Диагностика фоновой проверки внутри файла, втянутого в программу импортом (модуль, соседняя
  библиотека), отбрасывается: приписать её импортёру нельзя, а зачесть первому файлу батча (как
  делалось для редких cross-file случаев) значило бы красную метку на невиновном файле и
  превращение старых sloppy-`.js` модулей в ошибки у каждого `.ts`-импортёра. Ошибки *использования*
  модуля импортёром по-прежнему у импортёра; модуль-файл правил проверяется своим батчем.
- `Editor.ResolveModule` отдаёт только файлы под каталогом правил и каталогами модулей (рантайм
  разрешает абсолютный спецификатор куда угодно - правило и так читает что угодно через `spawn()`,
  а редактор не должен становиться универсальным читателем файлов); промахи редактора не логируются.
- Ретракт дубликата для примитивной причины (`throw "text"`, `throw 42`) - по тексту записи
  (у примитива нет идентичности), для объекта - по указателю.
- Чтение и транспиляция файла модуля из загрузчика (внутри `JS_Eval`) исключаются из окна
  watchdog'а (`excludeFromWatchdog`): медленный tsgo не должен выглядеть как runaway-цикл.
